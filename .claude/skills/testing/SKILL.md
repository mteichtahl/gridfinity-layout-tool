...
2. **`**kernel-tests**/**`is invisible to the root config.** Diagnostic/parity/perf kernel tests only run via`vitest.profile.config.ts`(forks pool, 1 worker, 600s timeout). They will silently not run in`pnpm run test:coverage` or CI unit shards.
