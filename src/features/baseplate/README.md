...
7. **Connectors sit at cell centers, not interior boundaries** — one connector per cell along each join edge (matching upstream `cutter_screw_together`), so a 1×1 piece pair still gets one. Computed by `cellCentersAlong()` in `cellDecomposition.ts` (worker generator) and a local mirror in `BaseplatePreview/snapClipPositions.ts` (preview, kept inline to respect the feature module boundary)
