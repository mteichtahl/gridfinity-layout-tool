...
describe('buildSlicerUrl', () => {
  it('builds a protocol URL with encoded file URL', () => {
    ...
    expect(result).toBe('prusaslicer://open?file=https%3A%2F%2Fexample.com%2Ffile.3mf');
  });
  ...
  it('encodes query parameters and ampersands in the file URL', () => {
    ...
    expect(result).toBe(
      'orcaslicer://open?file=https%3A%2F%2Fblob.vercel.com%2Fslicer-temp%2Fuuid.3mf%3Ffoo%3Dbar%26baz%3Dqux'
    );
  });

  it('uses the provided protocol as the scheme', () => {
    const result = buildSlicerUrl('bambustudio', 'https://example.com/file.3mf');
    expect(result).toMatch(/^bambustudio:\/\/open\?file=/);
  });
