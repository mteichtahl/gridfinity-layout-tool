...
describe('SelectDropdown', () => {
  it('renders all options', () => {
    render(<SelectDropdown value="cat1" onChange={vi.fn()} options={options} ariaLabel="Category" />);
    expect(screen.getAllByRole('option')).toHaveLength(2);
  });

  it('selects current value', () => {
    render(<SelectDropdown value="cat2" onChange={vi.fn()} options={options} ariaLabel="Category" />);
    expect(screen.getByRole('combobox')).toHaveValue('cat2');
  });

  it('calls onChange when selection changes', () => {
    const onChange = vi.fn();
    render(<SelectDropdown value="cat1" onChange={onChange} options={options} ariaLabel="Category" />);
  ...
  it('renders color swatch when provided', () => {
    const { container } = render(
      <SelectDropdown value="cat1" onChange={vi.fn()} options={options} colorSwatch="#ff0000" ariaLabel="Category" />
    );
