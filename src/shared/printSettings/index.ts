...
/**
 * Scale a baseline print time by the user's layer height and infill settings.
 *
 * Scaling model:
 ...
 *   Scale factor = 1 + 0.005 × (actual - baseline). At 15% this is 1.0;
 *   at 100% it's 1.425 (~42% more time for solid infill).
 *
...
export function scalePrintTime(baseMinutes: number, settings: PrintSettings): number {
  ...
  const infillScale = 1 + 0.005 * (settings.infillPercent - BASELINE_INFILL);
  return baseMinutes * layerScale * infillScale;
}
