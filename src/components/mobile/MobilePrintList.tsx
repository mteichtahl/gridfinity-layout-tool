...
export function MobilePrintList() {
  ...
  return (
    <div className="pb-4">
      {/* Summary */}
      <div className="p-4 rounded-lg mb-4 bg-surface-elevated">
      ...
      >
        {copyFeedback ? (
          <>
            <svg className="w-5 h-5 mr-2 text-[var(--color-success)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="space-y-2">
        {printRows.map((row) => (
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {row.needsSplit && (
                    <span className="px-1.5 py-0.5 rounded text-xs bg-[var(--color-warning-muted)] text-[var(--color-warning)]">
                      Split
                    </span>
                  )}
                </div>
                {row.labels[0] && (
                  <span className="text-sm truncate block text-content-tertiary">
                    {row.labels[0]}
                  </span>
                )}
              </div>
            ...
            {/* Split details */}
            {row.needsSplit && (
              <div className="mt-2 pt-2 text-sm border-t border-stroke-subtle text-content-tertiary">
