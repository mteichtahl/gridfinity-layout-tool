 ...
 */
export type CollectionValidationErrorCode =
  | 'VALIDATION_ERROR'
  | 'COLLECTION_FULL'
  | 'SIZE_LIMIT'
  | 'INVALID_ID';
...
export type CollectionValidationResult =
  ...
  | { valid: false; error: CollectionValidationError };

/**
 * Validate collection ID format (12 alphanumeric chars).
 */
 ...
 */
export function validateCollectionName(name: unknown): CollectionValidationResult {
  ...
  if (trimmed.length > COLLECTION_CONSTRAINTS.NAME_MAX_LENGTH) {
    return {
      valid: false,
      error: {
        code: 'VALIDATION_ERROR',
        message: `Collection name must be ${COLLECTION_CONSTRAINTS.NAME_MAX_LENGTH} characters or less`,
      },
    };
  }
  ...
  return {
    valid: true,
    name: sanitizeString(trimmed, COLLECTION_CONSTRAINTS.NAME_MAX_LENGTH),
  };
}

/**
 * Check if collection can accept more layouts.
 */
export function validateLayoutCount(currentCount: number): CollectionValidationResult {
  if (currentCount >= COLLECTION_CONSTRAINTS.MAX_LAYOUTS) {
    return {
      valid: false,
      error: {
        code: 'COLLECTION_FULL',
        message: `Collection has reached maximum of ${COLLECTION_CONSTRAINTS.MAX_LAYOUTS} layouts`,
      },
    };
  }
...
/**
 * Check if collection is nearing layout limit.
 */
export function isNearLayoutLimit(currentCount: number): boolean {
  return currentCount >= COLLECTION_CONSTRAINTS.MAX_LAYOUTS_WARNING;
}

/**
 * Calculate collection expiration date (2 years from now).
 */
export function calculateCollectionExpiration(): number {
  const now = Date.now();
  return now + COLLECTION_CONSTRAINTS.EXPIRATION_YEARS * 365 * 24 * 60 * 60 * 1000;
}

/**
 * Check if a collection has expired.
 */
export function isCollectionExpired(expiresAt: number): boolean {
  return Date.now() > expiresAt;
}

/**
 * Generate a unique layout name within a collection.
 * Appends (2), (3), etc. if name already exists.
 */
export function generateUniqueLayoutName(baseName: string, existingNames: string[]): string {
  const sanitized = sanitizeString(baseName, COLLECTION_CONSTRAINTS.NAME_MAX_LENGTH);
  ...
  const suffixPattern = new RegExp(`^${escapeRegex(sanitized)}\\s*\\((\\d+)\\)$`);
  let maxSuffix = 1;
  ...
  for (const name of existingNames) {
    ...
    } else if (name === sanitized) {
      maxSuffix = Math.max(maxSuffix, 2);
    }
  }

  const newName = `${sanitized} (${maxSuffix})`;
  // Ensure total length doesn't exceed limit
  if (newName.length > COLLECTION_CONSTRAINTS.NAME_MAX_LENGTH) {
    const suffix = ` (${maxSuffix})`;
    const maxBase = COLLECTION_CONSTRAINTS.NAME_MAX_LENGTH - suffix.length;
...
export { COLLECTION_CONSTRAINTS, SHARE_CONSTRAINTS };
